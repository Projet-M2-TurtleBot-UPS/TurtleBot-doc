...
\begin{center}
    ...
    A.1 & ?? & Marine Bouchet & 27/03/2017\\ \hline
    A.2 & ?? & Tristan Klempka & 28/03/2017\\ \hline
...
\item [Trajectoire] qui permet de générer le chemin à suivre pour aller d'un point A à un point B (cf. NAV
\item [Commande] qui permet d'assuer le suivi de trajectoire ou d'effectuer des commandes simples en boucle ouverte
...
Tout d'abord pour générer la carte de visibilité, il faut au préalablement avoir une carte de l'environnement -- créé virtuellement ou en utilisant la cartographie disponible sur le Turtlebot, utilisé dans le $map\_server$.
...
Le node $visib\_pgmwriter\_node.cpp$ ne doit pas être modifié, toutes les configurations se font directement dans le fichier $visib\_init.cpp$. En effet, le node lance la fonction $Ecriture\_carte\_visib()$ qui crée un fichier au format PGM (TODO SITE INTERNET) (TODO enchainment) l'ensemble des amers définit dans le $graph.xml$ se trouvant dans le dossier $/rsc$. Donc pour une taille de carte donnée et pour les configurations effectuées correctement dans $visib\_init.cpp$  -- expliquées par la suite, il suffit de modifier la position et l'orientation de nos amers dans le $graph.xml$ (TODO pas encore introduit) pour que la nouvelle carte de visibilité soit générée. automatiquement en lançant à nouveau notre node $visib\_pgmwriter\_node.cpp$.\\
...
- Le repère de la carte de l'environnement note R' qui a pour origine (-12.2 ; -13.8) (dans notre cas pour une carte 512x640) donc R' (X+12.2,Y+13.8).\\ (TODO REP?, À GÉNÉRALISER)
